...
export function createInboundHandler(deps) {
    const { cfg, botUserId, accountId, log, groupHistories, historyLimit, acceptBotMessage, dmPolicy, allowFrom, trustedGuilds, isUserInTrustedGuilds, } = deps;
    return async function handleTextChannelEvent(event) {
        try {
    ...
    async function handleTextChannelEventInner(event) {
        ...
        const storeAllowFrom = dmPolicy === 'pairing'
        ...
        const state = { streamingCard: null };
        let firstReplyBlockAt = null;
        let finalReplySentAt = null;
        ...
        log?.info?.(`[timing] total=${totalElapsedMs}ms llm=${llmElapsedMs}ms llm_share=${llmSharePercent}% first_reply=${firstReplyBlockAt ?? 'n/a'} chatType=${chatType} accountId=${accountId}`);
...
function parsePluginCommand(text) {
    ...
    if (!PLUGIN_COMMANDS.has(name))
        return null;
    return { name, args: match[2].trim() };
}
async function handlePluginCommand(cmd, ctx) {
...
async function handlePrintContext(ctx) {
...
function resolvePluginSessionRoute(ctx) {
...
async function readPluginSessionMessages(route) {
...
async function resetPluginSession(route) {
    const removedSessionFiles = await updateSessionStore(route.storePath, (store) => {
        ...
        return [[existing.sessionId, existing.sessionFile]];
    });
    await Promise.all(removedSessionFiles.map(async ([sessionId, sessionFile]) => {
...
function runtimeSafeLoadSessionStore(storePath) {
    return loadSessionStoreCompat(storePath);
}
function loadSessionStoreCompat(storePath) {
    return getKookRuntime().agent.session.loadSessionStore(storePath);
}
function resolvePluginTranscriptPath(route, sessionId, sessionFile) {
...
async function handleSessionReset(ctx) {
    const { deps, event, replyTarget } = ctx;
    let route;
    try {
        route = resolvePluginSessionRoute(ctx);
