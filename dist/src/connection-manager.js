...
export function getActiveClient(accountId) {
    return activeClients.get(accountId);
}
export function getFirstActiveClient() {
    for (const client of activeClients.values()) {
        return client;
    }
    return undefined;
}
export const kookGatewayAdapter = {
    async startAccount(ctx) {
        ...
        const historyLimit = Math.max(0, cfg.channels?.kook?.historyLimit ??
        ...
        const isUserInTrustedGuilds = async (userId, guildIds) => {
