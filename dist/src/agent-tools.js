...
async function handleUploadAsset(api, params) {
    if (!params.path) {
    ...
    return api.uploadAsset(formData);
}
...
export function createKookPlatformToolFactory() {
    return (ctx) => {
        const tool = {
            name: 'kook_platform',
            label: 'KOOK Platform',
            description: 'Execute KOOK platform REST API operations. ' +
            ...
            async execute(_toolCallId, args) {
                ...
                try {
                    ...
                    const result = await withTimeout(args.action === 'upload_asset'
                        ? handleUploadAsset(client.api, args.params ?? {})
                        : handler(client.api, args.params ?? {}), timeoutMs, `KOOK API request timed out after ${timeoutMs}ms while executing ${args.action}`);
                    if (!result.success) {
