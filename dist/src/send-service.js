...
export function createSendTarget(to, chatType = 'group') {
...
export function createSendService(client, botName) {
    const ensureDirectChat = async (userId) => {
    ...
    const sendCardPayload = async (target, content, replyToId) => {
        if (target.chatType === 'direct') {
            const chatCode = await ensureDirectChat(target.userId);
