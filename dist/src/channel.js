...
const kookMentionAdapter = {
    stripMentions(params) {
        return stripKookMentions(params.text);
    },
};
function parseKookExplicitTarget(raw) {
    ...
    if (/^user:/i.test(normalized)) {
        ...
        return id ? { to: `kook:user:${id}`, chatType: 'direct' } : null;
    }
    if (/^channel:/i.test(normalized)) {
        const id = normalized.replace(/^channel:/i, '').trim();
        return id ? { to: `kook:channel:${id}`, chatType: 'channel' } : null;
    }
    if (/^\d+$/.test(normalized)) {
        return { to: `kook:channel:${normalized}`, chatType: 'channel' };
    }
    return null;
}
const kookMessagingAdapter = {
    normalizeTarget(raw) {
...
function ensureDirectChatCode(client, userId) {
